...
const LAW_NAMES: Record<string, string> = {
  penal: 'قانون العقوبات المصري رقم 58 لسنة 1937',
  criminal_proc: 'قانون الإجراءات الجنائية المصري رقم 150 لسنة 1950',
  civil: 'القانون المدني المصري رقم 131 لسنة 1948',
  civil_proc: 'قانون المرافعات المدنية والتجارية المصري رقم 13 لسنة 1968',
  personal: 'قانون الأحوال الشخصية المصري',
};

function buildSystem(lawType: string): string {
  const lawName = LAW_NAMES[lawType] || 'القانون المصري';
  return `أنت كبير المستشارين القانونيين المصريين، خبير في ${lawName}. مهمتك تقديم تحليل احترافي تفصيلي لمواد ${lawName} وتعديلاته.
...
أعد الرد **حصرياً** بصيغة JSON صالحة بالشكل التالي بالضبط دون أي نص خارج JSON:
{
  ...
  "muzakkira": "نص مسودة المذكرة الكاملة (300-500 كلمة) بصياغة احترافية تستند للمادة وتذكر مبادئ النقض والدفوع الجوهرية"
}

قواعد صارمة:
- 5-7 دفوع شكلية، 5-7 دفوع موضوعية، 4-6 ثغرات، 3-5 مبادئ نقض، 3-5 مواد مرتبطة، 5-7 خطوات دفاع، 3-5 تعليقات.
- استخدم لغة قانونية مصرية رصينة مناسبة لفرع القانون.
- إن لم تتأكد من رقم طعن محدد فاكتفِ بـ "مبدأ مستقر - محكمة النقض المصرية".
...
const RATE_BUCKET_CLEANUP_INTERVAL = 5 * 60_000;
...
function rateLimit(ip: string): boolean {
  const now = Date.now();
  if (now - lastCleanup > RATE_BUCKET_CLEANUP_INTERVAL) {
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        const lawType = (body.law || 'penal').trim();
        let system = buildSystem(lawType);
        let userMsg = '';
        let cacheKey = '';
        ...
        if (mode === 'classify') {
          ...
          cacheKey = await hashFacts(facts);
        } else {
